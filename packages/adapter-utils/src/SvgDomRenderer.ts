...
export class SvgDomRenderer implements IRenderer {
  ...
  render(state: EditorState): void {
    ...
    while (this.contentLayer.firstChild) this.contentLayer.firstChild.remove();
  ...
  hitTest(point: Point, state: EditorState): NodeId | null {
    ...
    for (let i = children.length - 1; i >= 0; i -= 1) {
      const el = children[i];
      if (!el || !(el instanceof SVGGraphicsElement) || el.tagName === 'defs') continue;
      const id = el.getAttribute('data-id') ?? el.getAttribute('id');
      if (!id) continue;
      if (
        (el as SVGGeometryElement).isPointInFill?.(svgPoint) ||
        (el as SVGGeometryElement).isPointInStroke?.(svgPoint)
      ) {
        return id as NodeId;
      }
    }
  ...
  private renderOverlay(state: EditorState): void {
    ...
    while (this.overlayLayer.firstChild) this.overlayLayer.firstChild.remove();
    if (state.selection.ids.size === 0 || !this.contentLayer) return;
