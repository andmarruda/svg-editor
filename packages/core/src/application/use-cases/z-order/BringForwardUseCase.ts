...
export class BringForwardUseCase {
  ...
  execute(doc: Document, ids: NodeId[]): Document {
    const commands = ids.map((id) => {
      ...
      return new ReorderNodeCommand(id, Math.min(doc.rootOrder.length - 1, idx + 1));
    });
    return this.history.execute(new CompositeCommand('Bring forward', commands), doc);
  }
}
