  ...
  while (i < tokens.length) {
    ...
    switch (cmd) {
      ...
      case 'T': {
        ...
        commands.push({
          type: cmd as 'M' | 'L' | 'T',
          point: Point.of(nums.values[0] ?? 0, nums.values[1] ?? 0),
        });
...
export function serializePath(commands: readonly PathCommand[]): string {
  return commands
    .map((cmd) => {
      switch (cmd.type) {
        case 'M':
          return `M${fmt(cmd.point.x)},${fmt(cmd.point.y)}`;
        case 'L':
          return `L${fmt(cmd.point.x)},${fmt(cmd.point.y)}`;
        case 'H':
          return `H${fmt(cmd.x)}`;
        case 'V':
          return `V${fmt(cmd.y)}`;
        case 'C':
          return `C${fmt(cmd.cp1.x)},${fmt(cmd.cp1.y)} ${fmt(cmd.cp2.x)},${fmt(cmd.cp2.y)} ${fmt(cmd.point.x)},${fmt(cmd.point.y)}`;
        case 'S':
          return `S${fmt(cmd.cp2.x)},${fmt(cmd.cp2.y)} ${fmt(cmd.point.x)},${fmt(cmd.point.y)}`;
        case 'Q':
          return `Q${fmt(cmd.cp.x)},${fmt(cmd.cp.y)} ${fmt(cmd.point.x)},${fmt(cmd.point.y)}`;
        case 'T':
          return `T${fmt(cmd.point.x)},${fmt(cmd.point.y)}`;
        case 'A':
          return `A${fmt(cmd.rx)},${fmt(cmd.ry)} ${fmt(cmd.xRotation)} ${cmd.largeArc ? 1 : 0},${cmd.sweep ? 1 : 0} ${fmt(cmd.point.x)},${fmt(cmd.point.y)}`;
        case 'Z':
          return 'Z';
      }
    })
