...
import type { NodeId, BoundingBox } from '@svg-editor/core';
...
export function Canvas({ className, style }: CanvasProps) {
  ...
  const toDocCoords = useCallback((clientX: number, clientY: number) => {
    if (!containerRef.current) return { x: clientX, y: clientY };
    const rect = containerRef.current.getBoundingClientRect();
    const [a, , , d, e, f] = state.viewTransform.matrix;
